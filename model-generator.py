...
parser.add_argument("-port", "--port", help="Port", dest="port")
...
try:
    conn = psycopg2.connect("dbname='%s' user='%s' host='%s' port='%s' password='%s'" % (args.database, args.username, args.hostname, args.port, args.password))
